public class Line {
    ...
    public static final boolean[][] intersectionMap = new boolean[76][76];
    static {
        int count = 0;
        ...
        assert count == 76;
        // Precompute intersectionMap
        for (int i = 0; i < lines.length; i++) {
            for (int j = i + 1; j < lines.length; j++) {
                boolean intersect = lines[i].intersects(lines[j]);
                intersectionMap[i][j] = intersect;
                intersectionMap[j][i] = intersect;
            }
        }
    }
